//TRANSFORM DO GRUPO
//1) translate
//2) rotate (em torno da origem 0,0!)
//3) scale (também em torno da origem 0,0)
...
export const getMapPointFromPoint = (svgPoint, transform) => {
  ...
  // 2) unrotate
  const unrotated = transform.rotate
    ? rotatePoint(untranslated, -transform.rotate)
    : untranslated;

  // 3) unscale
  return {
    x: (unrotated.x - transform.x) / transform.scale,
    y: (unrotated.y - transform.y) / transform.scale,
  };
};
