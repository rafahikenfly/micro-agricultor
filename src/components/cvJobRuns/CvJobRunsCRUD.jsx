...
import CvJobRunsModal from "./CvJobRunsModal";


export default function CvJobRunsCRUD() {
  const { user } = useAuth();
  if (!user) return <NoUser />

  const [cvJobRuns, setCvJobRuns] = useState([]);
  ...
  useEffect(() => {
    ...
    const unsub = cvJobRunsService.subscribe((data) => {
      setCvJobRuns(data);
  ...
  return (
    <Container fluid>
      <Row className="mb-3">
        <Col>
          <Button variant="outline-success" onClick={criar}>+ Nova Tarefa de Visão Computacional</Button>
        </Col>
      </Row>
